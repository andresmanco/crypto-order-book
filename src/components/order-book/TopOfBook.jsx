...
export const TopOfBook = memo(function TopOfBook({ bestBid, bestAsk, isConnected }) {
  return (
    ...
    >
      <div>
        <p className="text-xs text-gray-300 uppercase">Best bid</p>
        <p className="text-xl py-1 text-bid font-bold">${(bestBid && formatPrice(bestBid.price)) || "---.--"}</p>
        <p className="text-xs text-gray-300 uppercase">Size</p>
      ...
      <div>
        <p className="text-xs  text-gray-300 uppercase text-right">Best ask</p>
        <p className="text-xl py-1 text-ask font-bold">${(bestAsk && formatPrice(bestAsk.price)) || "---.--"}</p>
        <p className="text-xs text-gray-300 uppercase text-right">Size</p>
